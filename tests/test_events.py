...
def test_tool_call_event():
    event = ToolCallEvent(
        tool_name="get_weather", tool_input={"city": "SF"}, call_id="call_123"
    )
...
def test_tool_result_event_success():
    event = ToolResultEvent(call_id="call_123", result={"temp": 72})
...
def test_tool_result_event_error():
    event = ToolResultEvent(call_id="call_123", result=None, error="Connection failed")
...
def test_collaborator_complete_event():
    ...
    event = CollaboratorCompleteEvent(
        agent_name="FinanceAgent", response="Revenue is $2.3M"
    )
    assert event.agent_name == "FinanceAgent"
