    ...
    try:
        ...
        if market_cap:
            if market_cap >= 1000000000000.0:
                market_cap_str = f"${market_cap / 1000000000000.0:.2f}T"
            elif market_cap >= 1000000000.0:
                market_cap_str = f"${market_cap / 1000000000.0:.2f}B"
            else:
                market_cap_str = f"${market_cap / 1000000.0:.2f}M"
        else:
            market_cap_str = "N/A"
        return {
            "symbol": symbol.upper(),
...
def analyze_volatility(symbol: str) -> dict:
    ...
    try:
        ...
        sharpe = (
            (avg_return - 0.05) / (returns.std() * 252**0.5) if returns.std() > 0 else 0
        )
        return {
            "symbol": symbol.upper(),
            "beta": round(beta, 2),
